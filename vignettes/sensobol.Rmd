...
```{r boot_replicas, fig.height=6.5, fig.width=6.5, fig.cap="Distribution of the bootstrap replicas."}

# Access boot replicas ------------------------------------

# Select only first and total-order indices
temp <- dt[1:k]

# Extract boot samples
b.rep <- temp[, "All":= list(lapply(V1, function(x) x["t"]))] %>%
  .[, .("Si" = lapply(All, function(x) lapply(x, function(y) y[, 1])), 
         "STi" = lapply(All, function(x) lapply(x, function(y) y[, 2]))), 
     parameters] %>%
  .[, lapply(.SD, unlist), .SDcols = c("Si", "STi"), parameters] %>%
  data.table::melt(., measure.vars = c("Si", "STi"))
